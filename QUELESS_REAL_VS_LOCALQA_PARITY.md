...
| Provider core entry | `.codex-real-vs-localqa-parity-20260730-121038/provider-access-real-profile-current.png` | Pending matched Local QA provider-role capture | Real app benchmark account shows a Free Provider badge and a hidden/off-screen `Create business profile` action, but no existing provider dashboard/profile is safely accessible without changing real-app data. | Real app account state limitation. Creating a business profile would mutate the protected benchmark package/data. | Do not alter `org.queless.app`; use Local QA provider fixtures for provider QA, and only compare real-provider screens if the user provides a real benchmark account/state that already has provider data. | Cannot compare safely without mutating real app. |
...
- Real-app provider core screens could not be benchmarked safely because the visible real account does not have an already-open provider dashboard/business profile state.
