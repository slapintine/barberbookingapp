...
async function createIndexes() {
  ...
  // Existing development databases may contain duplicate callback deliveries
  // from before webhook idempotency was enforced. Keep the newest copy of each
  // exact identity so the unique index can be added without blocking startup.
  await run(`
    ...
    WHERE id NOT IN (
      ...
      GROUP BY provider, event_type, reference, provider_reference
    )
    AND (COALESCE(reference, '') <> '' OR COALESCE(provider_reference, '') <> '')
  `);
  ...
  try {
    ...
    await run(`
      CREATE TABLE IF NOT EXISTS barbers (
        ...
        delivery_areas_json TEXT NOT NULL DEFAULT '[]',
        delivery_fee REAL DEFAULT NULL,
    ...
    await addColumnIfMissing("barbers", "pickup_available", `pickup_available INTEGER NOT NULL DEFAULT 1`);
    await addColumnIfMissing("barbers", "delivery_areas_json", `delivery_areas_json TEXT NOT NULL DEFAULT '[]'`);
    await addColumnIfMissing("barbers", "delivery_fee", `delivery_fee REAL DEFAULT NULL`);
    ...
    // Rich security/business audit trail (hashed IP/UA, redacted metadata).
    await run(`
