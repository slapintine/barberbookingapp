...
export const env = {
  ...
  androidAppVersion: (process.env.ANDROID_APP_VERSION || "1.0.9").trim(),
  androidVersionCode: Number(process.env.ANDROID_VERSION_CODE || 10),
  androidApkUrl: (process.env.ANDROID_APK_URL || "https://queless.org/downloads/queless-latest.apk").trim(),
  androidBuildId: (process.env.ANDROID_BUILD_ID || "").trim(),
  ...
  androidApkSha256: (process.env.ANDROID_APK_SHA256 || "").trim(),
  ...
  aiCoachMode: (process.env.AI_COACH_MODE || process.env.AI_PROVIDER || "gemini").trim().toLowerCase(),
  geminiApiKey: (process.env.GEMINI_API_KEY || "").trim(),
  geminiModel: (process.env.GEMINI_MODEL || "gemini-2.5-flash-lite").trim(),
  geminiBaseUrl: (process.env.GEMINI_BASE_URL || "https://generativelanguage.googleapis.com/v1beta").trim().replace(/\/+$/, ""),
  ...
  providerCoachDailyLimit: Math.min(100, Math.max(1, Number(process.env.PROVIDER_COACH_DAILY_LIMIT || 20))),
